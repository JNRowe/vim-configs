...
def configure(
    ...
    with file.open('w') as buildfile:
        ...
        n.build(
            str(location / 'README.html'), 'rst_compile', [
        ...
        rst_files = [
            p for p in location.glob('**/*.rst') if '.includes' not in p.parts
        ]

        if sphinx:
            n.build(
                str(location / '.build/html'),
                'sphinx_build', [
        ...
        for p in rst_files:
            if p.name == 'vimrc.rst':
                n.build(
                    str(location / p.with_suffix('')), 'rst_extract', [
                        str(location / p),
                    ], [
                ...
                'FAQ.rst', 'README.rst', 'background.rst', 'index.rst',
                'todo.rst'
            ):
                n.build(
                    str(location / p.with_suffix('.vim')), 'rst_extract', [
                        str(location / p),
                    ], [
                        str(location / 'rst2vim.py'),
                    ]

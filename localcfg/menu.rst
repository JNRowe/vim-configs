...
For |remind| and the excellent wyrd_ frontend to it::
    ...
    var remind_files = {}
    for fn in glob('~/.reminders.d/*', v:false, v:true)
        remind_files[fnamemodify(fn, ':t:gs?\.?_?')] = fn
    endfor
...
For X.org_’s main user configuration files::
    ...
    for fn in glob('~/.xinitrc.d/*', v:false, v:true)
        xorg_files[fnamemodify(fn, ':t:gs?\.?_?')] = fn
    endfor
    call menu#define_menu('&xorg', xorg_files)

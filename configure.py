...
def configure(
    ...
    local: Optional[bool] = typer.Option(False, help="Generate standalone build file."),
    colour: Optional[bool] = typer.Option(True, help="Generate coloured output."),
    vim: Optional[str] = typer.Option("vim", metavar="command", help="Path to vim."),
    ...
    with file.open("w") as buildfile:
        ...
        for p in chain(location.glob("spell/*.add"), location.glob(".meta/*.add")):
            n.build(
        ...
        rst_files = [
            ...
            if p.relative_to(location).parts[0] not in (".github", ".includes", "maybe")
        ]
        ...
        dein_cache_dir = xdg_cache_dir / "vim" / "dein" / "repos" / "github.com"
        n.build(
        ...
        n.build(
            ...
            variables={
                "exclude": " ".join(f"--exclude=$in/{d}" for d in libc_exclude),
                "lang": libc_langs,
                "extra_opts": "--recurse",
            },
        )
